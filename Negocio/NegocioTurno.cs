...
namespace Negocio
{
	public class NegocioTurno
	{
		...
		public void UpdateTurno(int idturno, int index)
		{
			...
			try
			{
				db.setearConsulta("UPDATE Turno SET  ESTADO=@estado, EstadoInforme=@estadoinf  WHERE ID_TURNO =@id_turno");
				db.setearParametro("@id_turno", idturno);
		...
		public void UpdateTurnoMed(Turnos turno)
		{
			...
			try
			{
				db.setearConsulta("UPDATE Turno SET EstadoInforme=@estadoinforme, ObservacionMedico=@obs  WHERE ID_TURNO =@id_turno");
		...
		public Turnos turnRepro(int idturno)
		{
			...
			try
			{
				db.setearConsulta("SELECT ID_TURNO, FECHA, ESTADO, ID_MEDICO, ID_PACIENTE,ID_HORA, ID_ESPECIALIDAD, observacion FROM TURNO WHERE ID_TURNO =@id_turno");
				db.setearParametro("@id_turno", idturno);
				db.ejecutarLectura();
				Turnos aux = new Turnos();
				if (db.Lector.Read())
