...
namespace Negocio
{
	public class NegocioPaciente
	{
		...
		public Paciente BuscarXId(int val)
		{
			DBConnection db = new DBConnection();
			Paciente aux = new Paciente();
			try
		...
		public Paciente BuscarXIdPaciente(int val)
		{
			DBConnection db = new DBConnection();
			Paciente aux = new Paciente();
			try
			{
				db.setearConsulta("SELECT ID_PACIENTE, NOMBRE, APELLIDO, DIRECCION, FECHA_NACIMIENTO, SEXO, ESTADO, TELEFONO, ID_USUARIO, DNI FROM PACIENTE WHERE ID_PACIENTE= @id");
				db.setearParametro("@id", val);
				db.ejecutarLectura();
				if (db.Lector.Read())
				{
